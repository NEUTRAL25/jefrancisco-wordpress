...
function construction_blocks_scripts() {
	...
	//font-awesome
	wp_enqueue_style( 'construction-blocks-fontawesome', get_template_directory_uri() . '/assets/font-awesome/css/all.css', array(), '5.15.3' );

	// script.js
	wp_enqueue_script('construction-blocks-main-script', get_template_directory_uri() . '/assets/js/script.js', array('jquery'), '1.0.0', true);
}
...
function construction_blocks_custom_admin_notice() {
    ...
    if (!get_user_meta(get_current_user_id(), 'dismissed_admin_notice', true)) {
        ...
        $construction_blocks_current_screen = get_current_screen();
        if ($construction_blocks_current_screen && $construction_blocks_current_screen->id !== 'appearance_page_construction-blocks-guide-page') {
            $construction_blocks_theme = wp_get_theme();
            ?>
            <div class="notice notice-info is-dismissible">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($construction_blocks_theme->get('Name')); ?></p>
